...
def SSIM_neural_output():
    ...
    for  root, folders, _ in os.walk(experiment_dir_cnn):
        ...
        for z in folders:
            im_experiment = misc.imread(os.path.join(experiment_dir_cnn, z +"/final_rec.png"))
            im_GT = misc.imread(os.path.join(GT_dir_cnn, z + "/target.png"))
...
def SSIM():
    ...
    for  root, _, files in os.walk(experiment_dir_simple_seg):
        ...
        for z in files:
            # print(z)
            im_experiment = misc.imread(os.path.join(experiment_dir_simple_seg, z))
            im_GT = misc.imread(os.path.join(GT_dir, z))
            ssim_error = ssim(im_experiment, im_GT)
            general_ssim.append(ssim_error)
...
def SSIM_between_classifiers():
    for  root, folders, _ in os.walk(experiment_dir_cnn):
        ...
        for z in folders:
            im_experiment_cnn = misc.imread(os.path.join(experiment_dir_cnn, z +"/final_rec.png"))
            im_GT = misc.imread(os.path.join(GT_dir_cnn, z + "/target.png"))
            ssim_error = ssim(im_experiment_cnn, im_GT)
            general_cnn_ssim.append(ssim_error)

            im_experiment_ss = misc.imread(os.path.join(experiment_dir_simple_seg, z +"png"))
...
if __name__ == '__main__':
    ...
    simple_seg = seg['ss']
    cnn_seg = seg['cnn']
    print("Simples Segmentation SSIM result: {}%".format(simple_seg)) # 0.7894920030495002
